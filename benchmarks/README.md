...
You may have noticed that the core of Anyhive Kit is built with Ruby on Rails, a framework that can scale from "Hello World" to IPO. Since Ruby's performance is not the best, we need to compare it with other frameworks to determine if it's the right choice.

The development philosophy of Anyhive Kit is: prioritize developer experience first, then pursue maximum performance. Don't get me wrong—performance is not secondary. Our goal is for the development team and community to focus on creating a better user experience, and then, through software engineering techniques, ensure that we also achieve optimal performance.
...
Anyhive Kit currently provides several core features:
